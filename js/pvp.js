...
function animate() {
  ...
  if (keys.arrowdown.pressed)
    player2.velocity.x = 0.3 * speedModifier + acceleration;
...
function check_player_boundary() {
  ...
  const box3 = new THREE.Box3().setFromObject(player2);
  if (box3.intersectsBox(box2)) {
    vb_z *= -1.1;
    do {
      if (vb_z > 0) ball.position.z += 1;
      else ball.position.z -= 1;
    } while (Math.abs(ball.position.z - player2.position.z) < 1.5);
    // if(vb_z>0.5) {
    //   vb_z *= -0.8;
    // } else {
    //   vb_z *= -1;
...
function updateCollision() {
  enemies.forEach((player2) => {
    let collision = Entity.boxCollision(player, player2);
    if (collision && !player2.collided) {
      console.log("hit");
      player2.velocity.z = player2.velocity.z * -1;
      player2.collided = true;

      if (player.velocity.x != 0) {
        player2.velocity.x += player.velocity.x / 2;
      }
    } else if (!collision) {
      player2.collided = false;
    }
    if (player2.position.x > 9 || player2.position.x < -9) {
      player2.position.x = player2.position.x > 0 ? 9 : -9;
      player2.velocity.x *= -1;
    }
    if (player2.position.z > 12 || player2.position.z < -12) {
      player2.position.z = player2.position.z > 0 ? 12 : -12;
